...
import "@fontsource/poppins"; // Impor font Poppins
...
const theme = createTheme({
  palette: {
    primary: { main: '#FFB6C1' }, // Warna pastel pink
    secondary: { main: '#87CEFA' }, // Warna pastel biru
    background: { default: '#FFF5F7' }, // Latar belakang lembut
  },
  typography: {
...
function Navbar(props) {
  ...
  const drawerContent = (
    <Box
      sx={{
        textAlign: 'center',
        background: 'linear-gradient(135deg, #FFB6C1, #FFD1DC)', // Gradasi pastel
        height: '100%',
        color: '#333',
        display: 'flex',
          ...
          <ListItem key={item.label} disablePadding>
            <ListItemButton
              ...
              sx={{
                ...
                px: 2,
                backgroundColor: location.pathname === item.path ? '#FFD1DC' : 'transparent',
                color: '#333',
                borderRadius: 2,
                '&:hover': {
      ...
      <Box sx={{ display: 'flex' }}>
        ...
        <Box
          ...
          sx={{
            flexGrow: 1,
            p: 3,
            ml: { sm: collapsed ? '60px' : `${drawerWidth}px` },
